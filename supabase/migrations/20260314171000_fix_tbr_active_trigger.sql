-- Migration: Fix TBR scan - allow re-loading of TBRs in insucesso
-- A TBR in "insucesso" (open piso_entry) is NOT considered occupied even if
-- its original ride is still active. The block only fires for TBRs in an active
-- ride that have NOT been marked as failed delivery.

-- 1. Update trigger function: only block duplicates that are NOT in insucesso
...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE UPPER(rt.code) = UPPER(NEW.code)
      ...
      -- Allow if TBR is in insucesso (piso_entry open) - it failed and is being retried
      AND NOT EXISTS (
    ...
    RAISE EXCEPTION 'Este TBR ja esta vinculado a outro carregamento ativo';
  END IF;
  RETURN NEW;
END;
$$;

-- 2. Update process_tbr_scan RPC with the same insucesso-aware logic
...
BEGIN
  ...
  IF EXISTS (
    ...
    RETURN jsonb_build_object('success', false, 'error', 'Este TBR ja foi finalizado no PS');
  END IF;
  ...
  -- If in insucesso, the TBR failed delivery and is being retried - allow it through.
  IF v_in_active_ride AND NOT v_in_insucesso THEN
    RETURN jsonb_build_object('success', false, 'error', 'Este TBR ja esta vinculado a outro carregamento ativo');
  END IF;

  -- 4. Calculate trip_number (count previous distinct rides this TBR was in)
  ...
  -- 5. Close any open piso_entries and rto_entries for this TBR
  ...
  -- 6. Check for local duplicate (same ride)
  SELECT EXISTS (
